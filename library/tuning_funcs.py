...
def elitism_tuning(mut_func, mut_prob, xo_func, xo_prob, selection_algo,
                   n_runs: int = 30, max_gen:int = 100, pop_size: int = 100):
    ...
    for elitism in [True, False]:
        ...
        runs_results = []
        for run in range(1, n_runs+1):
            initial_population = [LUGASolution(mutation_function = mut_func, crossover_function = xo_func)
                                            for i in range(pop_size)]
        ...
        current_comb = pd.DataFrame()
        current_comb['Elitism'] = [elitism for _ in range(current_comb_all_runs.shape[0])]
        one_comb = pd.concat([current_comb.reset_index(drop=True), current_comb_all_runs.reset_index(drop=True)], axis=1)
        comb_results.append(one_comb)
        
    final_results = pd.concat(comb_results).reset_index(drop=True)
    final_filepath = HERE.parent / "combination_search"/ f"elistim_exp.csv"
    final_results.to_csv(final_filepath, index=False)
