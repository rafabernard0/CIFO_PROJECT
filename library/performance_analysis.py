...
import pandas as pd
import plotly.graph_objects as go
import plotly.express as px  # NEW ─ colour palette


# ─────────────────────────────────────────────────────────────────────────────
# helper: convert Plotly’s hex colour to an rgba string with configurable alpha
...
def plot_Xbest_per_exp(
    df_exps: dict, Xbest: int = 4, metric: str = "MBF", std: bool = False
):

    # 1) read and unify the experiment CSVs
    all_df_exps = []
    for exp_name, description in df_exps.items():
        df_exp = pd.read_csv(f"combination_search/final_results_{exp_name}.csv")
        df_exp["Experience Name"] = exp_name
        df_exp["Experience Description"] = description
        all_df_exps.append(df_exp)

    xbf_df = pd.concat(all_df_exps)

    # 2) aggregate metric (median for MBF, mean for ABF)
    if metric == "MBF":
        by_comb_exp = (
            xbf_df.groupby(
                [
    ...
    if std:
        std_df = (
            ...
            .std()
            .reset_index()
            .rename(columns={"Fitness": "std"})
        )

        by_comb_exp = by_comb_exp.merge(
            std_df,
            on=[
                "Combination",
                "Combination ID",
    ...
    # 4) keep the X best combinations per experiment
    ...
    filtered_data = [
        by_comb_exp[
            (by_comb_exp["Combination ID"] == comb["Combination ID"])
    ...
    for idx, ((comb_id, comb, exp_name, exp_desc), group) in enumerate(grouped):
        ...
        fig.add_trace(
            go.Scatter(
                ...
                hovertemplate=(
                    f"Combination: {comb}<br>"
                    f"Experiment: {exp_name}<br>"
                    "Generation: %{x}<br>"
                    "Fitness: %{y}<extra></extra>"
                ),
            )
        )

        # std-band (upper + lower, filled in-between)  ──────────────
        if std:
            # invisible upper “roof”
            fig.add_trace(
                go.Scatter(
                    x=group["Generation"],
                    y=group["y_upper"],
            ...
            # lower edge that closes the area
            fig.add_trace(
                go.Scatter(
                    x=group["Generation"],
                    y=group["y_lower"],
                    mode="lines",
                    fill="tonexty",
                    fillcolor=_hex_to_rgba(color, 0.15),  # same hue, 15 % opacity
                    line=dict(color=color, width=0),
                    showlegend=False,
                )
            )
